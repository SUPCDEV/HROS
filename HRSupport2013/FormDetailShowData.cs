...
{
    public partial class FormDetailShowData : Form
    {
        SqlConnection con = new SqlConnection(DatabaseConfig.ServerConStr);


        public FormDetailShowData(string DocId , string OutId)
        {
            InitializeComponent();
        ...
        private void FormDetailShowData_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                string sql = @"SELECT [OutOfficeId] ,[EmplId],[EmplFname] + ' ' +[EmplLname] As EmplFullName,[Dimention],[Dept]
                                        ...
                                        ,[HrApprovedOutName],[HrApprovedOutDateTime]
                                        ,CASE [HrApprovedIn] WHEN '1' THEN 'รออนุมัติ' WHEN '2' THEN 'อนุมัติ' ELSE 'ไม่มีข้อมูล' END AS [HrApprovedIn]
                                        ,[HrApprovedInName],[HrApprovedInDateTime]


                                  FROM [IVZ_HROUTOFFICE] 
                                  WHERE [OutOfficeId] = '" + radLabelOutId.Text.ToString() + "' AND  [DocId] = '" + radLabelDocId.Text.ToString() + "' ";
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        ...
                        radLabelHrStatustIn.Text = reader["HrApprovedIn"].ToString();
                        radLabelHrApproveIn.Text = reader["HrApprovedInName"].ToString();
                        radLabelHrDateTimeIn.Text = reader["HrApprovedInDateTime"].ToString();
