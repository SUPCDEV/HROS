...
{
    public partial class FormDetailHeadApproved : Form
    {
        SqlConnection con = new SqlConnection(DatabaseConfig.ServerConStr);

        public FormDetailHeadApproved(string DocId, string OutId )
        {
            InitializeComponent();
        ...
        private void Form5_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                string sql = @"SELECT [OutOfficeId] ,[EmplId],[EmplFname] + ' ' +[EmplLname] As EmplFullName,[Dimention],[Dept]
                                        ...
                                        ,[TruckId],[Reason],[CreatedName],CreatedDateTime 
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        ...
                        radLabelEmplCreate.Text = reader["CreatedName"].ToString();
                        radLabelDateTime.Text = reader["CreatedDateTime"].ToString();
                        break;
                    }
                }
            }
