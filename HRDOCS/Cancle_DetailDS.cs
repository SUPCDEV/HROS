...
namespace HRDOCS
{
    public partial class Cancle_DetailDS : Form
    {
        ...
        public Cancle_DetailDS(string DS_DocId)
        {
            DS_id = DS_DocId;
            InitializeComponent();
                ...
                foreach (GridViewDataRowInfo row in rgv_StatusDoc.Rows)
                {
                    if (row.Index > -1)
                    {
                        ...
                        if (row.Cells["CHECK"].Value != null)
                        {
                            ...
                            sqlCommand.Parameters.AddWithValue(string.Format(@"DOCREFER{0}", row.Index), DS_id);
