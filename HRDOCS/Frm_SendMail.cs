...
namespace HRDOCS
{
    public partial class Frm_SendMail : Form
    {
        ...
        public Frm_SendMail()
        {
            ...
            ddl_Employee.DropDownListElement.AutoCompleteSuggest.SuggestMode = Telerik.WinControls.UI.SuggestMode.Contains;
        }
        ...
        private void btn_SendMail_Click(object sender, EventArgs e)
        {
           ...
           // WSSendEmail(@"noreply@hros.supc.net", @"jittakorn@supercheapphuket.com", @"", "รายการพิจารณาอนุมัติการเปลี่ยนวันหยุดออนไลน์", sb_mailBody.ToString());
            WSSendEmail(@"noreply@hros.supc.net", txt_To.Text.ToString(), txt_CCTo.Text.ToString(), "รายการพิจารณาอนุมัติการเปลี่ยนวันหยุดออนไลน์", sb_mailBody.ToString());
        }
        //Send To... Not have CC...
            ...
            try
            {
                using (SqlCommand sqlCommand = new SqlCommand())
                {
                    ...
                    sqlCommand.CommandText = string.Format(
                            @" SELECT (RTRIM(PWFNAME)+'  ' +RTRIM(PWLNAME) ) AS PWFULLNAME 
                                FROM [PWEMPLOYEE]
                                ORDER BY [PWFNAME] ");
                    ...
                    if (dataTable.Rows.Count > 0)
                    {
                        ...
                        ddl_Employee.DataSource = dataTable;
                        ddl_Employee.ValueMember = "PWEMPLOYEE";
                        ddl_Employee.DisplayMember = "PWFULLNAME";
                    }
        ...
        private void ddl_Employee_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == 13)
            {
