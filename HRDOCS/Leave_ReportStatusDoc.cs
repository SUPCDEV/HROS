...
namespace HRDOCS
{
    public partial class Leave_ReportStatusDoc : Form
    {
                ...
                using (SqlCommand sqlCommand = new SqlCommand())
                {
                    ...
                    if (Rb_Section.IsChecked == true)
                    {
                        sqlCommand.CommandText = string.Format(@"select  convert(varchar,DATEDIFF(year, PWSTARTDATE, getdate()) - (CASE WHEN DATEADD(year, DATEDIFF(year, PWSTARTDATE, getdate()), PWSTARTDATE) > getdate() THEN 1 ELSE 0 END))+'ปี '+
                                                                ...
                                                                where SPC_CM_LEAVEHD.SECTIONID = '{0}' AND SPC_CM_LEAVEHD.DOCDATE BETWEEN '{1}' and '{2}' ", Cbb_Section.SelectedValue.ToString(), Dtp_Start.Value.Date.ToString("yyyy-MM-dd"), Dtp_End.Value.Date.ToString("yyyy-MM-dd"));
                                                                ...
                                                                left join 
                                                                        (
	                                                                        ...
	                                                                        group by PWEMPLOYEE.PWEMPLOYEE,PWADJTIME.PWEVENT
					                                                            ...
					                                                            AND SPC_CM_LEAVEDT.LEAVETYPE = LEAVE.PWEVENT
                                                                where SPC_CM_LEAVEHD.EMPLID = '{0}' AND SPC_CM_LEAVEHD.DOCDATE BETWEEN '{1}' and '{2}' ", Txt_EmplID.Text, Dtp_Start.Value.Date.ToString("yyyy-MM-dd"), Dtp_End.Value.Date.ToString("yyyy-MM-dd"));
                                                            ...
                                                            left join 
                                                                        (
	                                                                        ...
	                                                                        group by PWEMPLOYEE.PWEMPLOYEE,PWADJTIME.PWEVENT
					                                                            ...
					                                                            AND SPC_CM_LEAVEDT.LEAVETYPE = LEAVE.PWEVENT
                                                            where SPC_CM_LEAVEHD.DLDOCNO = '{0}' ", Txt_DocNo.Text);
                    }
        ...
        void exporter_ExcelTableCreated(object sender, Telerik.WinControls.UI.Export.ExcelML.ExcelTableCreatedEventArgs e)
        {
            ...
            string headerText = "รายงานการลาประจำวันที่:  " + Dtp_Start.Text + " ถึง " + Dtp_End.Text;
