...
namespace HRDOCS
{
    public partial class Chd_ReportStatusDoc : Form
    {
        ...
        private void Chd_ReportStatusDoc_Load(object sender, EventArgs e)
        {
            this.dt_FromSec.Text = DateTime.Now.AddDays(-7).Date.ToString("yyyy-MM-dd");
            this.dt_ToSec.Text = DateTime.Now.ToString("yyyy-MM-dd");
            //this.dt_FromEmp.Text = DateTime.Now.ToString("yyyy-MM-dd");
            //this.dt_ToEmp.Text = DateTime.Now.ToString("yyyy-MM-dd");
                ...
                if (rdb_Section.ToggleState == ToggleState.On)
                {
                    ...
                    sqlCommand.CommandText = string.Format(
                        @" SELECT HD.DOCID ,CONVERT(VARCHAR, HD.TRANSDATE , 23) AS TRANSDATE
                           ...
                           AND HD.DOCSTAT != 0 ORDER BY HD.DOCID DESC "
            ...
            try
            {
                if (rdb_EmpId.ToggleState == ToggleState.On)
                {
                    ...
                    sqlCommand.CommandText = string.Format(
                        ...
                        //, dt_FromEmp.Value.Date.ToString("yyyy-MM-dd")
                        //, dt_ToEmp.Value.Date.ToString("yyyy-MM-dd")
                        //, ClassCurUser.LogInEmplId  
                        );
