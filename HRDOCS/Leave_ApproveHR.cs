...
namespace HRDOCS
{
    public partial class Leave_ApproveHR : Form
    {
        ...
        private void InitializeDataGrid()
        {
            ...
            Dg_HRApprove.Columns.Add(DLDOCNO);
        ...
        void Dg_HRApprove_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                using (Leave_ApproveHR_Detail frm = new Leave_ApproveHR_Detail(Dg_HRApprove.Rows[e.RowIndex].Cells["DLDOCNO"].Value.ToString()))
                {
                    frm.Text = "บุคคลอนุมัติใบลา";
                ...
                if (dataTable.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dataTable.Rows.Count; i++)
                    {
                        Dg_HRApprove.Invoke(new EventHandler(delegate
                        {
                            ...
                            Dg_HRApprove.Rows[Dg_HRApprove.RowCount - 1].Cells["DLDOCNO"].Value =
            ...
            try
            {
                ...
                using (SqlCommand sqlCommand = new SqlCommand())
                {
                    ...
                    else if (Rb_DocNo.IsChecked == true)
                    {
                        sqlCommand.CommandText = string.Format(@"select ltrim(rtrim(PWEMPLOYEE.PWFNAME)) + ' ' + ltrim(rtrim(PWEMPLOYEE.PWLNAME)) as CREATEDNAME,* from SPC_CM_LEAVEHD
                                                                left join PWEMPLOYEE on SPC_CM_LEAVEHD.createdby = PWEMPLOYEE.PWEMPLOYEE
                                                                where SPC_CM_LEAVEHD.DLDOCNO = '{0}'  AND DOCSTAT != 0 AND HEADAPPROVED = 1 AND HRAPPROVED = 0 
                                                                AND SECTIONID IN (select PWSECTION from SPC_CM_AUTHORIZE
